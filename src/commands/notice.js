...
module.exports = {
	data: new SlashCommandBuilder()
		.setName('notice')
		.setDescription('Make a notice about something.')
        ...
        .addChannelOption(option => option
            .setName('channel')
            .setDescription('The channel to send the notice to.')
            .setRequired(true)
        )
        .addStringOption(option => option
            .setName('title')
            .setDescription('The title of the notice')
            .setRequired(true)
        )
        .addStringOption(option => option
            .setName('text')
            .setDescription('The contents of the notice')
            .setRequired(true)
        ),
	...
	exec: async(client, interaction) => {
        ...
        collector.on('collect', async i => {
            ...
            if (i.customId === 'confirm') {
                ...
                channel.send({ embeds: [new MessageEmbed().setAuthor(title).setDescription(text).setColor(0x53DD6C).setFooter(`Notice by: ${interaction.member.displayName}`)]})
                if (ping != 'none') {
                    const message = channel.send({ content: ping }).then(msg => msg.delete({ timeout: 500 }))
                }
            } else if (i.customId === 'cancel') {
                await interaction.followUp({ content: `Notice was cancelled.`, ephemeral: true })
            }
        })
